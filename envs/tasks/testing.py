...
PUT_BIN_SPECS = EasyDict({
        "objects": ["wooden_spoon", "wooden_spoon@base", "butter", "butter@base"],
        "butter": butter_dict,
        "cookies": cookies_dict,
        "wooden_spoon": wooden_spoon_dict,
        "butter@base": butter_base_dict,
        "wooden_spoon@base": wooden_spoon_base_dict, 
        "target_region": target_region_dict,
    })

def testing(exp_name="normal", *args, **kwargs):
    task_specs = PUT_BIN_SPECS
    if exp_name == "normal":
        pass

    elif exp_name == "distracting":
        task_specs["objects"] = ["wooden_spoon", "wooden_spoon@base", "butter", "butter@base", "cookies"]

    elif exp_name == "placement":
        task_specs["wooden_spoon"]["xrange"] = [[0.05, 0.10], [0.05, 0.10]]
        task_specs["wooden_spoon"]["yrange"] = [[-0.12, -0.10], [0.10, 0.12]]
        ...
        pass
    return testinglol(task_specs=task_specs,
                             *args,
                             **kwargs)
...
class testinglol(BaseDomain):
    ...
    def _check_success(self):
        ...
        cream_pos = self.sim.data.body_xpos[self.obj_body_id["wooden_spoon"]]
        cream_base_pos = self.sim.data.body_xpos[self.obj_body_id["wooden_spoon@base"]]
        ...
        cream_stacked = (np.abs((cream_pos - cream_base_pos)[:2]) <= 0.03).all() and self.check_contact(self.objects_dict["wooden_spoon@base"], self.objects_dict["wooden_spoon"])
